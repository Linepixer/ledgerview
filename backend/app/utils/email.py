...
def send_account_deletion_email(to_email: str, token: str) -> None:
    """
    Send an account deletion confirmation link to the user.
    """
    app_url = os.getenv("FRONTEND_URL", "http://localhost:3000")
    app_url = app_url.rstrip('/')
    deletion_link = f"{app_url}/account/delete-confirm?token={token}"
    ...
    <head>
        ...
        <style>
            ...
            .button {{
                display: inline-block;
                background-color: #ef4444; /* Red button for danger */
                color: #ffffff;
                text-decoration: none;
                font-weight: 600;
                padding: 12px 28px;
    ...
    <body>
        <div class="container">
            ...
            <p>
                Hemos recibido una solicitud para eliminar permanentemente tu cuenta y todos los datos asociados. Esta acción no se puede deshacer. Haz clic en el botón de abajo si deseas continuar y borrar tu cuenta. El enlace es válido por 1 hora.
            </p>
            <a href="{deletion_link}" class="button">Eliminar mi cuenta permanentemente</a>
    ...
    if smtp_user and smtp_password:
        try:
            ...
            with smtplib.SMTP_SSL(smtp_host, smtp_port) as server:
                server.login(smtp_user, smtp_password)
                server.send_message(msg)
            print(f"Account deletion email sent to {to_email} via SMTP.")
        except Exception as e:
            print(f"Error sending email: {e}")
    else:
        # Mock mode
